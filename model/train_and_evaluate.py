...
def train_and_save():
    ...
    print("data loaded and preprocessed", preprocess_data)
    ...
    print("Baseline model & evaluation:", {"mae": baseline_mae, "rmse": baseline_rmse})
    ...
    metrics = {"mae": mae, "rmse": rmse,"baseline_mae":baseline_mae , "baseline_rmse":baseline_rmse,"timestamp": datetime.now().isoformat()}
    pd.DataFrame([metrics]).to_json(
        METRIC_PATH, orient="records", indent=2
    )
